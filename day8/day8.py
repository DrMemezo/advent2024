...
def get_antinode_pos(base_pos:tuple[int,int], man_dist:tuple[int,int]) -> tuple[int,int]|None:
    global MAP_X_LEN, MAP_Y_LEN

    new_man_distance = tuple(map(lambda pos: pos * 2, man_dist))
...
def main():
    ...
    for ant_curr in antennas:
        for ant_pair in antennas:
            ...
            if antinode_pos := get_antinode_pos(ant_curr.position, man_distance):
